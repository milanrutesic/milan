...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	function display_button($name, $position = 'up') /* {{{ */
	{
		...
		if ($name == 'goto_text') {
			...
			// some js here.... on enter submit, on click erase ?...
		...
		if ($name == 'goto_combo') {
			...
			//  add onchange="return this.form.submit();" DONE ???
	...
	function list_table() /* {{{ */
	{
		...
		while ((!$fetched && ($row = $this->sql_fetch($res)) != false)
				|| ($fetched && $row != false)) {
			$fetched = false;
			echo '<tr onMouseOver="this.className=\'highlight\'" onMouseOut="this.className=\'normal\'" class="',$this->getCSSclass('row', null, 'next'),'">',"\n";
			if ($sys_cols) { /* {{{ */
				$key_rec     = $row['qf'.$this->key_num];
				...
				if ($select_recs) {
					...
					if ($this->nav_graphic_links()) {
						...
						if ($this->view_enabled()) {
							...
							echo 'pme-view.png" height="32" width="32" border="0" ';
						...
						if ($this->change_enabled()) {
							...
							echo $css_class_name,'" src="',$this->url['images'];
							echo 'pme-change.png" height="32" width="32" border="0" ';
						...
						if ($this->copy_enabled()) {
							...
							echo $css_class_name,'" src="',$this->url['images'];
							echo 'pme-copy.png" height="32" width="32" border="0" ';
						...
						if ($this->delete_enabled()) {
							...
							echo $css_class_name,'" src="',$this->url['images'];
							echo 'pme-delete.png" height="32" width="32" border="0" ';
